...
interface DocumentFile {
  ...
  expiryDate?: string;
  createdAt: string;
  // demo: store base64 data URL
  dataUrl?: string;
}

// Save documents to localStorage (with base64 file content)
function getDocs(): DocumentFile[] {
  try { return JSON.parse(localStorage.getItem('documents') || '[]'); } catch { return []; }
}
function saveDocs(docs: DocumentFile[]) { localStorage.setItem('documents', JSON.stringify(docs)); }
...
const Documents: React.FC = () => {
  ...
  const load = () => setDocuments(getDocs());
  useEffect(() => { load(); }, []);
  ...
  const handleOpenUpload = () => {
    setUploadName(''); setCategory('ID Proof'); setExpiryDate(''); setSelectedFile(null); setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setUploading(true);
    // Read file as base64 data URL for demo storage
    ...
    reader.onload = () => {
      ...
      docs.push(newDoc);
      saveDocs(docs);
      setShowModal(false); setUploading(false); load();
    };
  ...
  const handlePreview = (doc: DocumentFile) => {
    if (doc.dataUrl) { const w = window.open(); w?.document.write(`<img src="${doc.dataUrl}" style="max-width:100%"/><br/><a href="${doc.dataUrl}" download="${doc.name}">Download</a>`); }
  };

  const handleDownload = (doc: DocumentFile) => {
    if (!doc.dataUrl) return;
    const link = document.createElement('a');
    link.href = doc.dataUrl; link.download = doc.name;
      ...
      <div className="dashboard-card !p-0">
        <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row gap-4 items-center justify-between bg-slate-50/30">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input type="text" placeholder="Search document name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-100">
                <th className="px-6 py-4">Document Name</th><th className="px-6 py-4">Category</th>
                <th className="px-6 py-4">Uploaded On</th><th className="px-6 py-4 text-center">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {filteredDocs.length > 0 ? filteredDocs.map((doc) => (
                <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-red-500 shrink-0"><FileText size={18} /></div>
                      <div>
                  ...
                  <td className="px-6 py-4 text-xs font-bold text-slate-500">
                    ...
                    {doc.expiryDate && <span className="block text-[9px] text-red-500 font-medium">Expires: {new Date(doc.expiryDate).toLocaleDateString('en-IN')}</span>}
                  </td>
                ...
                <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-400 italic text-sm">No files uploaded. Click "Upload Document" to add files.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-lg w-full overflow-hidden relative z-10">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1"><label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Category</label>
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className="input-field bg-white">
                    ...
                    <option value="Insurance">Insurance</option><option value="Others">Others</option>
                  </select></div>
                <div className="space-y-1"><label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Expiry Date (Optional)</label>
                  <input type="date" value={expiryDate} onChange={(e) => setExpiryDate(e.target.value)} className="input-field" /></div>
              </div>
              <div className="pt-4 border-t border-slate-100 flex gap-4 justify-end">
                ...
                <button type="submit" disabled={uploading} className="btn-primary cursor-pointer disabled:opacity-50">{uploading ? 'Uploading...' : 'Upload File'}</button>
              </div>
            </form>
          </div>
        </div>
