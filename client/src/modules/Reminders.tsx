...
import { Plus, Search, Trash2, X, Bell, Loader2 } from 'lucide-react';
...
const Reminders: React.FC = () => {
  ...
  // Poll for active reminders to trigger browser notifications
  useEffect(() => {
    const checkAndTriggerNotifications = () => {
      const now = new Date();
      reminders.forEach((rem) => {
        const remDateObj = new Date(rem.date);
        // If due in the next minute and not already notified
        const diffMs = remDateObj.getTime() - now.getTime();
        const diffMins = diffMs / (1000 * 60);
        
        if (diffMins >= 0 && diffMins <= 1) {
          if (Notification.permission === 'granted') {
            new Notification(`Life Admin Alert: ${rem.title}`, {
              body: rem.description || `Your ${rem.type} reminder is due now!`,
              icon: '/favicon.svg'
            });
          }
        }
    ...
    const interval = setInterval(checkAndTriggerNotifications, 60000); // Check every minute
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error saving reminder details');
    }
  };
  ...
  const handleToggleComplete = (id: string) => {
    if (completedIds.includes(id)) {
      ...
      // Autodelete completed reminders after a short delay for clean checklist flow
      setTimeout(() => {
        handleDelete(id);
      }, 1500);
    }
            ...
            {filteredReminders.length > 0 ? (
              filteredReminders.map((rem) => {
                ...
                return (
                  <div 
                    key={rem.id} 
                    className={`flex items-center justify-between p-4 transition-all rounded-lg group ${
                      isCompleted ? 'opacity-40 line-through bg-slate-50/50' : 'hover:bg-slate-50/50'
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      />
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center bg-slate-50 text-slate-500`}>
                        <Bell size={20} />
                      </div>
                      <div>
                        <h4 className="text-sm font-bold text-slate-800 leading-tight flex items-center gap-2">
                          {rem.title}
                          <span className={`text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded border ${getPriorityStyles(rem.priority)}`}>
                            {rem.priority}
                          </span>
                        </h4>
                    ...
                    <button 
                      ...
                      className="p-1.5 hover:bg-red-50 text-red-500 rounded transition-all cursor-pointer opacity-0 group-hover:opacity-100"
                      title="Delete"
                    >
                      <Trash2 size={16} />
                    </button>
              ...
              <div className="p-12 text-center text-slate-400 italic text-sm">No reminders scheduled.</div>
            )}
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-lg w-full overflow-hidden relative z-10">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Reminder Title</label>
                <input 
                  type="text" 
                  value={title} 
