...
import { Plus, Search, Edit2, Trash2, X, ShieldCheck, HeartPulse, Car, Loader2 } from 'lucide-react';
...
const Insurance: React.FC = () => {
  ...
  const handleOpenEdit = (policy: InsurancePolicy) => {
    ...
    setInsuranceType(policy.insuranceType);
    setCoverageAmount(policy.coverageAmount.toString());
    setPremiumAmount(policy.premiumAmount.toString());
    setStartDate(new Date(policy.startDate).toISOString().split('T')[0]);
    setEndDate(new Date(policy.endDate).toISOString().split('T')[0]);
    setRenewalDate(new Date(policy.renewalDate).toISOString().split('T')[0]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error saving policy details');
    }
  };
  ...
  // Helper: calculate days remaining and badge color
  const getDaysInfo = (renewalDateStr: string) => {
    const diffTime = new Date(renewalDateStr).getTime() - new Date().getTime();
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    
    if (diffDays < 0) {
      return { text: 'Expired', color: 'bg-red-50 text-red-600 border-red-100', isExpired: true, expiresSoon: false };
    }
    if (diffDays <= 30) {
      return { text: `${diffDays} days left`, color: 'bg-orange-50 text-orange-600 border-orange-100', isExpired: false, expiresSoon: true };
    }
      ...
      <div className="dashboard-card !p-0">
        <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row gap-4 items-center justify-between bg-slate-50/30">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input
              type="text"
              placeholder="Search policy name or provider..."
              value={searchQuery}
        ...
        {loading ? (
          ...
          <div className="p-5 space-y-4">
            {filteredPolicies.length > 0 ? (
                ...
                return (
                  <div key={policy.id} className="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-xl border border-slate-100 hover:bg-slate-50/50 transition-all gap-4">
                    <div className="flex items-center gap-5">
                      <div className="w-12 h-12 rounded-lg flex items-center justify-center bg-slate-50 text-primary">
                        <PolicyIcon size={28} />
                      </div>
                      <div>
                    ...
                    <div className="flex flex-wrap items-center justify-between md:justify-end gap-6 md:gap-12">
                      ...
                      <div className="text-left md:text-right min-w-[120px]">
                        ...
                        <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded border inline-block mt-1 ${daysInfo.color}`}>
                          {daysInfo.text}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          ...
                          className="p-1.5 hover:bg-red-50 text-red-500 rounded transition-all cursor-pointer"
                          title="Delete"
                        >
                          <Trash2 size={16} />
                        </button>
              ...
              <div className="p-12 text-center text-slate-400 italic text-sm">No insurance policies found.</div>
            )}
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-lg w-full overflow-hidden relative z-10">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <h3 className="font-bold text-slate-800 text-lg">
                {modalMode === 'add' ? 'Add Insurance Policy' : 'Edit Policy Details'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Policy Name</label>
                  <input 
                    type="text" 
                    value={policyName} 
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Policy Number</label>
                  <input 
                    type="text" 
                    value={policyNumber} 
                    onChange={(e) => setPolicyNumber(e.target.value)} 
                    placeholder="SHI123456" 
                    className="input-field" 
                    required 
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Coverage Amount (₹)</label>
                  <input 
                    type="number" 
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Premium Amount (₹)</label>
                  <input 
                    type="number" 
                    value={premiumAmount} 
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">Renewal Date</label>
                  <input 
                    type="date" 
                    value={renewalDate} 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Nominee Details</label>
                  <input 
                    type="text" 
