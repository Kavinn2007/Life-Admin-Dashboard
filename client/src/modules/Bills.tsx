...
const Bills: React.FC = () => {
  ...
  const handleOpenEdit = (bill: Bill) => {
    ...
    setPaymentMethod(bill.paymentMethod || 'Credit Card');
    setNotes(bill.notes || '');
    setShowModal(true);
  };
  ...
  const handleMarkAsPaid = async (bill: Bill) => {
    try {
      await api.put(`/bills/${bill.id}`, {
        ...bill,
        status: 'PAID',
        paymentMethod: bill.paymentMethod || 'Net Banking',
      });
      fetchBills();
    } catch (error) {
      ...
      <div className="dashboard-card !p-0">
        <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row gap-4 items-center justify-between bg-slate-50/30">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input
              type="text"
              placeholder="Search bill or provider..."
              value={searchQuery}
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredBills.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-400 italic text-sm">No bills found matching filters.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-lg w-full overflow-hidden relative z-10">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <h3 className="font-bold text-slate-800 text-lg">
                {modalMode === 'add' ? 'Add New Bill' : 'Edit Bill Details'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  >
                    {['Electricity', 'Water', 'Internet', 'Gas', 'Mobile', 'Others'].map((cat) => (
              ...
              <div className="pt-4 border-t border-slate-100 flex gap-4 justify-end">
                ...
                >
                  {modalMode === 'add' ? 'Add Bill' : 'Save Changes'}
                </button>
              </div>
            </form>
          </div>
